...
export default function ToolRow({ toolKey, toolData, onChange, onRemove }) {
  ...
  return (
    <div className="group flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 py-3.5 px-4 rounded-xl hover:bg-sea-dark/25 transition-colors border-b border-sea-medium/15 last:border-b-0">
      
      {/* Tool Name + Plan */}
      <div className="flex items-center justify-between sm:flex-1 min-w-0 gap-2 w-full">
        ...
        <button
          type="button"
          onClick={onRemove}
          className="p-1.5 text-sea-medium hover:text-red-400 transition-all rounded-lg cursor-pointer sm:opacity-0 sm:group-hover:opacity-100 flex-shrink-0"
          title="Remove"
        >
